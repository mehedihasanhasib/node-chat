...
app.post("/auth", function (request, response) {
  ...
  if (email && password) {
    connection.query(
      "SELECT * FROM users_table WHERE email = ? AND pwd = ?",
      [email, password],
...
app.get("/get_user_id", (request, response) => {
  connection.query(
    `SELECT * FROM users_table WHERE email='${request.session.email}'`,
    (error, results) => {
      if (error) throw error;
...
app.post("/store-message", (req, res) => {
  ...
  connection.query(
    `INSERT INTO messages (message, user_id, role, created_at) VALUES ('${message}', '${user_id}', '${role}', UNIX_TIMESTAMP(${Date.now()}))`,
